...
function scheduler_schedule_result_status(array $result): string
{
    ...
    if ($message === 'ロック取得失敗のためスキップ' || $message === '楽天アプリID / アクセスキー未設定のためスキップ') {
        return 'skipped';
    }
    return 'success';
}
...
function scheduler_run_schedule(array $schedule): array
{
    ...
    return match ($type) {
        'items' => scheduler_run_items_schedule(rakuten_sync_service('items'), $settings),
        default => ['synced_count' => 0, 'message' => '未対応スケジュールです'],
    };
}

function scheduler_run_items_schedule(RakutenSyncService $service, array $settings): array
{
    ...
    $extraParams = [
        ...
        'keyword' => trim((string)($settings['keyword'] ?? 'アダルトグッズ')) ?: 'アダルトグッズ',
    ];
    if ($compoundKeyword !== '') {
    ...
    try {
        $result = $service->syncItemsBatch(
            settings_allowed_item_sync_batch((int)($settings['item_sync_batch'] ?? 30)),
            $offset,
            $extraParams,
            $excludeKeywords
        );
...
function scheduler_skip_missing_credentials(PDO $pdo, string $jobKey): ?array
{
    ...
    if (trim((string)($cred['api_id'] ?? '')) !== '' && trim((string)($cred['access_key'] ?? '')) !== '') {
        return null;
    }

    $message = '楽天アプリID / アクセスキー未設定のためスキップ';
