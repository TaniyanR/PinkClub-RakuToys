...
function installer_normalize_settings_table(PDO $pdo, string $stepLabel): void
{
    ...
    if (in_array('api_id', $columns, true)) {
        ...
        $sql = 'INSERT INTO `' . $tmpTable . '`(setting_key, setting_value, created_at, updated_at) SELECT "rakuten_application_id", COALESCE(CAST(api_id AS CHAR), ""), NOW(), NOW() FROM settings' . $orderBy . 'LIMIT 1 ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value), updated_at=NOW()';
        $pdo->exec($sql);
    }

    if (in_array('affiliate_id', $columns, true)) {
        $orderBy = in_array('id', $columns, true) ? ' ORDER BY id ASC ' : '';
        $sql = 'INSERT INTO `' . $tmpTable . '`(setting_key, setting_value, created_at, updated_at) SELECT "rakuten_affiliate_id", COALESCE(CAST(affiliate_id AS CHAR), ""), NOW(), NOW() FROM settings' . $orderBy . 'LIMIT 1 ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value), updated_at=NOW()';
        $pdo->exec($sql);
    }
