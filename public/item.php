...
if ($desc === '') {
    $desc = item_pick_raw_text($raw, ['itemCaption', 'catchcopy', 'comment', 'description', 'caption', 'story', 'introduction']);
}
if ($desc === '') {
...
$rawMakerName = item_pick_raw_text($raw, ['shopName']);
$productCode = item_pick_raw_text($raw, ['itemCode']);
if ($productCode === '') {
    $productCode = trim((string)($item['content_id'] ?? ''));
}
$priceText = trim((string)($item['price_min_text'] ?? ''));
$reviewCount = max(0, (int)($raw['reviewCount'] ?? $item['review_count'] ?? 0));
$reviewAverage = max(0, (float)($raw['reviewAverage'] ?? $item['review_average'] ?? 0));
...
$deliveryStartText = item_pick_raw_text($raw, ['updateTimestamp', 'date', 'deliveryStartDate', 'delivery_start_date']);
...
if ($productIdDisplay === '') {
    $productIdDisplay = trim((string)($raw['itemCode'] ?? ''));
}
...
$pageDescriptionSource = $desc !== '' ? $desc : $title . 'の楽天市場商品ページ。価格・レビュー・ショップ情報を紹介します。';
  ...
  <section class="pcf-detail pcf-item-main">
    ...
    <div class="pcf-item-main__info">
      <table style="width:100%; border-collapse:collapse; border:0; color:#000 !important; font-size:12px;">
        <tbody>
          <tr><th style="text-align:left; font-weight:700; padding:4px 8px 4px 0; white-space:nowrap; border:0;">商品コード</th><td style="padding:4px 0; border:0;"><?= e($productCode !== '' ? $productCode : '―') ?></td></tr>
          <tr><th style="text-align:left; font-weight:700; padding:4px 8px 4px 0; white-space:nowrap; border:0;">価格</th><td style="padding:4px 0; border:0;"><?= e($priceText !== '' ? $priceText : '―') ?></td></tr>
          <tr><th style="text-align:left; font-weight:700; padding:4px 8px 4px 0; white-space:nowrap; border:0;">ショップ</th><td style="padding:4px 0; border:0;"><?= $makerLinks !== [] ? implode('、', $makerLinks) : e($rawMakerName !== '' ? $rawMakerName : '―') ?></td></tr>
          <tr><th style="text-align:left; font-weight:700; padding:4px 8px 4px 0; white-space:nowrap; border:0;">レビュー</th><td style="padding:4px 0; border:0;"><?= $reviewCount > 0 ? e(number_format($reviewAverage, 2) . ' / 5（' . number_format($reviewCount) . '件）') : '―' ?></td></tr>
