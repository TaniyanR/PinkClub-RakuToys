...
$accessKey = (string)($cred['access_key'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        api_credential_set($apiType, $apiId, $accessKey, $affiliateId);
        $message = '設定を保存しました。';
        $messageType = 'success';
    }
    ...
    if ($action === 'test_save') {
        try {
            ...
            $accessKey = trim((string)post('access_key', $accessKey));
            $affiliateId = trim((string)post('affiliate_id', $affiliateId));
            api_credential_set($apiType, $apiId, $accessKey, $affiliateId);
            $sync = rakuten_sync_service($apiType);
            ...
            while ($attempts < 12) {
                ...
                $result = $sync->syncItemsBatch(
                    10,
                    $nextOffset,
                    ['sort' => $sort, 'keyword' => (string)$s['keyword']]
                );
...
<section class="card">
  <h1><?= e($pageTitle) ?></h1>
  <p>PinkClub-RakuToysでは楽天商品検索APIだけを使用します。アプリIDとアクセスキーは必須、アフィリエイトIDは成果リンク生成に使用します。</p>
  ...
  <form method="post" class="stack" style="max-width:700px;">
    <?= csrf_input() ?>
    <div>
      <label>楽天アプリID<br><input type="text" name="api_id" value="<?= e($apiId) ?>" style="width:100%"></label>
    </div>
    <div>
      <label>アクセスキー<br><input type="password" name="access_key" value="<?= e($accessKey) ?>" autocomplete="new-password" style="width:100%"></label>
    </div>
    <div>
      <label>楽天アフィリエイトID（推奨）<br><input type="text" name="affiliate_id" value="<?= e($affiliateId) ?>" style="width:100%"></label>
    </div>
